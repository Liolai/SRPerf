...
class ConfigParser(object):
        ...
        for config in configs:
            ...
            self.configs.append(
                Config(
                    ...
                    rate=config["rate"],
                    run=config["run"],
                    start_tx_rate=config["start_tx_rate"] if "start_tx_rate" in config else 1,
                    line_rate=config["line_rate"] if "line_rate" in config else LINE_RATES[config["experiment"]],
                    ...
                    duration=config["duration"] if "duration" in config else 10
                )
            )
    ...
    def get_packet(config):
        return "%s-%s-%s" % (config.type, config.experiment, ConfigParser.MAPPINGS[config.size])
