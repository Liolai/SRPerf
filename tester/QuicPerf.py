...
from QuicDriver import *
from Experiment import *


class QuicPerfOutput(ExperimentOutput):
...
class QuicPerfDriver:
    ...
    def doPerformanceTest(self):
        ...
        # We initialize the QuicDriver used to drive multiple tests with the
        # same traffic rate.
        driver = QuicDriver(self.server, self.txPort, self.rxPort, self.pcap, self.rate, self.duration)
    ...
    def doPostProcessing(self, results):
        # We create 'QuicPerfOutput' object which contains a reference to
        ...
        if 0 == dlRuns.size:
            ...
            return QuicPerfOutput(results, txRate, 0, 0)
        ...
        output = QuicPerfOutput(results, txRate, dlMean, dlStd)
        return output

    # Run is reentrant and it can be called multiple times without creating
    # a new QuicPerfDriver instance.
    def run(self):
        runs = []
        output = None
...
class QuicExperiment(Experiment):
    def __init__(self, server, txPort, rxPort, pcap, rate, repetitions, duration):
        ...
        self.perfDriver = QuicPerfDriver(server, txPort, rxPort, pcap, rate, repetitions, duration)
        self.invoked = False
...
# Experiment Factory for Quic
class QuicExperimentFactory(ExperimentFactory):
    ...
    def build(self, txRate):
        return QuicExperiment(self.server, self.txPort, self.rxPort, self.pcap, txRate, self.repetitions, self.duration)


# Entry point used for testing
if __name__ == "__main__":
    factory = QuicExperimentFactory("127.0.0.1", 0, 1, "pcap/raw-pcap-files/plain-ipv6-64.pcap", 1, 5)
    print("Running ...")
    ...
    print("Requested Tx Rate {0}, Mean {1}, Std. {2}".format(output.getRequestedTxRate(), output.getAverageDR(), output.getStdDR()))
